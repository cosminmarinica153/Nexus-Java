...
public class BlackJack {
    ...
    public void startBet() {
        for (int i = 0; i < players.length - 1; i = i + 1) {
            ...
            session.dispatch(
                    new Dispatcher("You have " + players[i].getCredits() + " credits", session.getDestiantion(i)));
            session.dispatch(
    ...
    private void dealerTurn(Player currPlayer) {
        if (currPlayer.getCards().size() == 2) {
            session.broadcast(new Dispatcher(
                    currPlayer.getType() + ": " + currPlayer.getCards() + "(" + currPlayer.checkTotal() + ")"));
